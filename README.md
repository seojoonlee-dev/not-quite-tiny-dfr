...
| `OnClick` | string | unset | What a tap does. `"Action"` runs the button's `Action`/keys; `"Expand"` expands the button in place — reusing the slider's animation — and shows `ExpandCommand`'s output until it idles (see [Expandable widgets](#expandable-widgets)). Any other string is a shell command: on a `Media` widget it runs when the now-playing panel is tapped outside the transport controls, e.g. `"systemd-run --user gtk-launch tidal"` to raise the player app (`systemd-run --user` launches it outside the daemon's sandbox, in your session). Only `"Action"` **lights up on tap** (the pressed fill); `"Expand"` (which has its own animation) and buttons with no `OnClick` stay flat when tapped. |
